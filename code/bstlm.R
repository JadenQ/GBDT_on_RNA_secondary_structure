...
xgbLinearGrid<-expand.grid(nrounds=c(500),lambda=c(3),alpha=c(3),
                           eta=c(0.04))   #best for ph
...
#test PCA BEST!
  gbm2=caret::train(data.frame(X_train), Y_train, method = "pcr", preProcess = "scale", 
  weights = NULL,trControl=trControl,metric = "RMSE",tuneGrid=pcrGrid)
